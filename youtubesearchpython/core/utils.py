...
def format_view_count(view_count_str: Optional[str]) -> dict:
    """
    Formats view count into text and short format.
    
    Args:
        view_count_str: View count as string
        
    Returns:
        dict with 'text' (comma-formatted with 'views') and 'short' (K/M/B notation)
    """
...
def format_duration(seconds_str: Optional[str]) -> dict:
    ...
    Args:
        seconds_str: Duration in seconds as string
        
    Returns:
        dict with 'seconds' (int) and 'text' (MM:SS format)
    """
    if not seconds_str:
...
def format_published_time(publish_date: Optional[str]) -> Optional[str]:
    """
    Formats publish date to relative time string (e.g., "2 years ago").
    
    Args:
        publish_date: ISO format date string
        
    Returns:
        Relative time string or None if date cannot be parsed
    """
    if not publish_date:
        return None
